...
*/
 ...
 ///yoooDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
           // Inicializar UART
    

    while (1) {
        // Leer datos desde UART
        char *message = readuart();
        if (message == NULL) {
            ESP_LOGE("UART", "Error: No se pudo leer el mensaje");
            continue;
        }
        printf("Received: %s\n", message);
        ...
        if (token1 != NULL && token2 != NULL && token3 != NULL) {
            if (strcmp(token1, "temp") == 0) {
                ...
                ESP_LOGE("UART", "Error: Invalid message format");
            }
        }

        // Liberar memoria asignada para el mensaje
        free(message);
 ///yooDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD

        // Apagar LEDs
        update_led(LED1.CHANEL_R, "RED", 0);
        update_led(LED1.CHANEL_G, "GREEN", 0);
        update_led(LED1.CHANEL_B, "BLUE", 0);

        // Esperar 5 segundos (TIMEOFF)
        vTaskDelay(TIMEOFF *1000/ portTICK_PERIOD_MS);
        
       

        // Encender LEDs
        update_led(LED1.CHANEL_R, "RED", 150);
        update_led(LED1.CHANEL_G, "GREEN", 150);
        update_led(LED1.CHANEL_B, "BLUE", 150);
        

        // Esperar el tiempo de encendido (TIMEON)
        vTaskDelay(TIMEON *1000/ portTICK_PERIOD_MS);
...
/////
 ...
 static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
 {
     ...
     uint16_t duty_value = adc1_get_raw(ADC1_CHANNEL_3);
     ...
     sprintf(dhtSensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}",temperature, 1.0);
 ...
 /// new servo registros aquii
 static esp_err_t http_server_current_time_handler(httpd_req_t *req) {
    time_t now;
    struct tm timeinfo;
    char timeString[50];
    ...
    char timeJSON[100];
    sprintf(timeJSON, "{\"time\":\"%s\"}", timeString);
 ...
 static esp_err_t http_server_set_servo_angle_handler(httpd_req_t *req) {
    ...
    if (httpd_req_recv(req, angle_str, sizeof(angle_str) - 1) <= 0) {
    ...
    if (angle < 0 || angle > 180) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Angle must be between 0 and 180");
        return ESP_FAIL;
    }
...
static esp_err_t http_server_set_automatic_mode_handler(httpd_req_t *req) {
    ...
    uint16_t duty_value = adc1_get_raw(ADC1_CHANNEL_0);
    float voltage = (duty_value * 3.3) / 4095;
    int angle = (voltage / 3.3) * 180;
...
// servomotor
static esp_err_t http_server_servo_handler(httpd_req_t *req) {

    ESP_LOGI(TAG, "/servo.json requested");
	...
	valueRangeMax_str = strdup(valueRangeMax_json->valuestring);
    int angle = atoi(valueRangeMin_str);
    servo_set_angle(angle);
	cJSON_Delete(root);
	...
	ESP_LOGI(TAG, "Received color: %s and range in %s to %s", color_str,valueRangeMin_str,valueRangeMax_str);
...
//servomotor
static esp_err_t http_server_configure_uart_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "/configure_uart.json requested");

    char uart_data[128];

    if (httpd_req_recv(req, uart_data, sizeof(uart_data) - 1) <= 0) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to read request body");
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "Received UART data: %s", uart_data);
 ...
 static esp_err_t http_server_potenciometro_json_handler(httpd_req_t *req)
 {	
     ESP_LOGI(TAG, "/potenciometro.json requested");
     ...
     float voltage = (duty_value * 3.3) / 4095;
     printf("Voltage potenciometro: %f\n", voltage);
     float potenciometro_por = (voltage *1000)/3.3-voltage;
 
 
     
 
 
     char potenciometroJSON[100];
 
     sprintf(potenciometroJSON, "{\"pot\":\"%.1f\"}",voltage);
 
     httpd_resp_set_type(req, "application/json");
     httpd_resp_send(req, potenciometroJSON, strlen(potenciometroJSON));
 ...
 static esp_err_t http_server_updateRangeColor_handler(httpd_req_t *req)
 {
     ESP_LOGI(TAG, "/updateRangeColor.json requested");
	...
	valueRangeMin_str = strdup(valueRangeMin_json->valuestring);
	valueRangeMax_str = strdup(valueRangeMax_json->valuestring);
    TIMEON = atoi(valueRangeMin_str);
    TIMEOFF = atoi(valueRangeMax_str);
	cJSON_Delete(root);
 ...
 static esp_err_t http_server_updateINTColor_handler(httpd_req_t *req)
 {
     ESP_LOGI(TAG, "/updateINTColor.json requested");
     ...
     if (color_json == NULL || !cJSON_IsString(color_json) || valueRangeMin_json == NULL || !cJSON_IsString(valueRangeMin_json) ) {
         cJSON_Delete(root);
         // Missing or invalid JSON fields
         //httpd_resp_send_err(req, HTTP_STATUS_BAD_REQUEST, "Missing or invalid JSON data fields");
         ESP_LOGI(TAG, "Missing or invalid JSON data fields");
     ...
     ESP_LOGI(TAG, "Received color: %s and intensity in %s", color_str,valueRangeMin_str);
     if (strstr(color_str, "red") != NULL)
     {
         // Set the red led
         ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_5 , (atoi(valueRangeMin_str)*255/100));
         ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_5);
     }
     else if (strstr(color_str, "green") != NULL)
     {
         // Set the green led
         ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_4, (atoi(valueRangeMin_str)*255/100));
         ledc_update_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_4);
     }
     else if (strstr(color_str, "blue") != NULL)
     {
         // Set the blue led
         ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_3, (atoi(valueRangeMin_str)*255/100));
 ...
 static httpd_handle_t http_server_configure(void)
 {
     ...
     if (httpd_start(&http_server_handle, &config) == ESP_OK)
     {
         ...
         //URIS USUARIO
         httpd_uri_t servo_json = {
            .uri = "/servo.json",
            .method = HTTP_POST,
            .handler = http_server_servo_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &servo_json );
 


         httpd_uri_t updateRangeColor_json = {
            .uri = "/updateRangeColor.json",
            .method = HTTP_POST,
            .handler = http_server_updateRangeColor_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &updateRangeColor_json );
 
             //uri INTESIDDADES
         httpd_uri_t updateINTColor_json = {
                 .uri = "/updateINTColor.json",
                 .method = HTTP_POST,
                 .handler = http_server_updateINTColor_handler,
                 .user_ctx = NULL
             };
         httpd_register_uri_handler(http_server_handle, &updateINTColor_json );
        
 
         httpd_uri_t potenciometro_json = {
                 .uri = "/potenciometro.json",
                 .method = HTTP_GET,
                 .handler = http_server_potenciometro_json_handler,
                 .user_ctx = NULL
         };
             
         httpd_register_uri_handler(http_server_handle, &potenciometro_json);
